...
- product token for `User-Agent`: `scipaper-harness`, the application's own name ([identity decision](../architecture/2026-09-24-scipaper-independent-identity.md))
- version: read from the owning package's manifest via `createRequire`, never a hand-copied constant
- app URL: `https://github.com/M-24rjgc/SciPaper-Harness` - the repository home
...
| Direct DeepSeek endpoint | `User-Agent` for app attribution; the conditional `x-deepseek-harness-session-id` describes the request, and no user identifier is sent. Do not send OpenRouter-only headers unless DeepSeek documents an equivalent contract. |
