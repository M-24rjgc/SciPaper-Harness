...
- 安装包增加约 440 KB 的索引；每张图在第一次被浏览或保存时下载一次，约 100 KB。
- 图片缓存之前，浏览和保存需要联网；索引和检索可以离线使用。
- 覆盖范围是 2023 至 2026 年的机器学习与 AI 会议；其他领域回退到 agent 自己找到的 arXiv 论文。
- 更新配图库要用 `scripts/build_figure_gallery.py` 在更新的版本上重新构建索引。
