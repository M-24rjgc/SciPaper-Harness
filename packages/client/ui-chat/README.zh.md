...
已发送的文件引用及消息日志确认调用的 skill 可在右侧栏打开预览。文件路径使用当前查看的 Session；skill 名称由该 Session 当前的输入触发源解析。两者悬停或聚焦时均使用正文文件链接的虚线下划线。会话、目录和命令标签仍只作为引用展示。Markdown 图片中的 POSIX 绝对路径和 Windows 盘符绝对路径均通过已认证的宿主文件接口加载；文件访问权限和媒体限制仍由宿主执行。
