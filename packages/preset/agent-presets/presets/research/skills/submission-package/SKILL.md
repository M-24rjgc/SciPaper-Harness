...
2. **Venue requirements**: the official template (`research_artifact` apply-template with the venue — stage final for a camera-ready version — or import-template for files the user supplies), page limit, anonymisation for double-blind review (no author names, acknowledgements or self-identifying links), required sections (limitations, broader impact, reproducibility checklist) where the venue asks for them. `template/<venue>/GUIDE.md` summarises the venue's rules; confirm them in this year's call for papers.
