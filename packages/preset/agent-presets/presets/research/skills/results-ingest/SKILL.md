---
name: results-ingest
description: Use when results already exist (from-results mode) or after experiments — to bring measured data into the project as evidence, align the claims with what the data actually shows, and produce every derived number with a script.
---
...
5. **Record provenance** for the key claims: `claim` with an exact quote (the number) at its locator (the CSV line or JSON key).

## Done when

The `ingest` phase is done: at least one current data source exists, and every number you intend to report is present in imported data or `facts.json`.
