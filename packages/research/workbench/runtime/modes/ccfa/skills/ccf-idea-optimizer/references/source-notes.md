...
Use this file when the user asks why the idea-optimization workflow is structured this way, asks for origin, or needs current official policy checks.
...
- For literature-grounded idea evolution, use the public-system records in the shared registry as workflow origin only. Current relevant records include `ai-scientist-v2`, `rd-agent`, `storm`, `gpt-researcher`, `paper-qa2`, `ai-researcher`, and `arbor-agent`. Do not present their GitHub popularity or claimed benchmark performance as evidence that an idea is scientifically valid.
