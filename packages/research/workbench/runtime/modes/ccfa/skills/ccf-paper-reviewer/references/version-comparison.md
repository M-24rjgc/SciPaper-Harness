...
The Markdown report can be checked directly with `--report --mode version-comparison`, using the fixed two-scorecard subheadings in `fixed-output-format.md`. This checks presentation and the generic current-readiness table; the original `validate(data)` comparison checks still handle arithmetic and score-decrease origin when structured data is available internally. Neither route requires a persisted JSON file.
