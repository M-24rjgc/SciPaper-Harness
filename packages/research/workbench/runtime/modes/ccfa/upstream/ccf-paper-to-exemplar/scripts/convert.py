...
def _check_pymupdf():
    # [research-workbench] pypdfium2 ships with the platform Python; pymupdf stays an option.
    for module in ('pymupdf', 'pypdfium2'):
        try:
            __import__(module)
            return
        except ImportError:
            continue
    print('ERROR: neither pymupdf nor pypdfium2 is installed. Run: pip install pypdfium2', file=sys.stderr)
    sys.exit(1)


def _page_texts(pdf_path):
    # [research-workbench] one text string per page, from pymupdf when present, else pypdfium2.
    try:
        import pymupdf
    except ImportError:
        import pypdfium2
        ...
        try:
            for page in document:
                ...
                yield textpage.get_text_range().replace('\r\n', '\n').replace('\r', '\n')
                textpage.close()
                page.close()
        finally:
            document.close()
...
def extract_text(pdf_path):
    parts = []
    for i, t in enumerate(_page_texts(pdf_path), 1):
        if t.strip():
