...
| WebSearch + `fetch_reference_figures.py` (grounding) | `research_media` find-reference-figures first: the built-in gallery of hand-reviewed Figure 1s from ICLR, ICML, NeurIPS, CVPR, ACL and AAAI, searched with an English query for the topic and `pattern` for the figure type (`tier` award or oral for the most recognised); fetch-reference-figures `{galleryIds, label}` saves the chosen ones under `figures/refs/` with a `.source.json` naming each paper. When the gallery has nothing on topic, find the paper with `research_evidence` literature-search or the web search tool and fetch-reference-figures `{arxivIds, label}`. Look at the saved candidates with `read_image` |
...
3. Ground it: take the main figure of an on-topic top-venue paper — from the figure gallery first, then from a paper you find — and pass the chosen image as `references`. Grounding is mandatory for schematic types; if a genuine multi-query search of the gallery and the literature finds none, stop and tell the user.
